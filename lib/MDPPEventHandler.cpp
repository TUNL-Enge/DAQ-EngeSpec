...
void MDPPEvent::read_banks(TMEvent* event){
	...
	for (auto m : this->module_map) {
		...
		if (midas_bank){
			// pull the data out
			uint32_t* dMDPP = (uint32_t*)event->GetBankData(midas_bank);
			...
			this->parse_data(module_num, m.second.nchannels, dMDPP, nMDPP);
		}
	}
}

void MDPPEvent::parse_data(int module_num, int nchannels, uint32_t* dMDPP, int nMDPP){
	...
	for (int i=0; i < nMDPP; i++){
		...
		if ((((data_word >> 30) & bitmasks::two_bit) == 0) &&
			...
			case 0 :
				...
				mdpp_bank.adc[raw_channel] = (data_word & bitmasks::sixteen_bit);
				break;
		
			case 1 :
				// TDC value
				mdpp_bank.tdc[(raw_channel - nchannels)] =  (data_word & bitmasks::sixteen_bit);
				break;
